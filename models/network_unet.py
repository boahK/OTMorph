...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool3d(2),
            DoubleCv(in_channels, out_channels)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleCv(in_channels, out_channels, in_channels // 2)
        else:
            self.up = nn.ConvTranspose3d(in_channels , in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleCv(in_channels, out_channels)
    ...
    def forward(self, x1, x2):
        ...
        x1 = F.pad(x1, [diffX_dv2, diffX - diffX_dv2,
                        diffY_dv2, diffY - diffY_dv2])
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, base_factor=32, bilinear=True):
        ...
        self.inc = DoubleCv(n_channels, base_factor)
